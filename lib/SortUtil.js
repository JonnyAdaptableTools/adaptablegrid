var SortUtil = {
  ...
  columnToIndexes: function (ind) {
    ...
    for (var i=1; i<this.cells.length; i++) {
      indexOfValue = this.columnValueToIndex[ind][this.cells[i].getCell(ind).getValue()];
      this.cells[i].getCell(ind).setValue(indexOfValue);
    }
  ...
  indexesToColumn: function (ind) {
    ...
    for (var i=1; i<this.cells.length; i++) {
      this.cells[i].getCell(ind).setValue(this.columnIndexToValue[ind][this.cells[i].getCell(ind).getValue()]);
    }
  ...
  configureRowCol: function () {
    ...
    for (i=0; i<this.height; i++) {
      for (j=0; j<this.width; j++) {
        this.cells[i].getCell(j).row = i;
        this.cells[i].getCell(j).col = j;
      }
    }
