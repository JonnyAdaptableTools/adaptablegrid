...
var SortUtil = {
  ...
  events: function () {
    ...
    $(this).find('.Grid-header').addClass('Grid-sortable');
    $(this).find('.Grid-header').on('click', SortUtil.clickColumn.bind(this));
  ...
  clickColumn: function (header) {
    ...
    if ($(header.target).hasClass('Grid-sort-asc')) {
      c = 'Grid-sort-des';
    }
    else {
      c = 'Grid-sort-asc';
    ...
    var s = new Sorter([{ column: this.columns[columnIndex], order: (c == 'Grid-sort-asc') }]);
    
    s.process(this, function () {
      this.cellToElement(0, columnIndex).addClass('Grid-sort').addClass(c);
