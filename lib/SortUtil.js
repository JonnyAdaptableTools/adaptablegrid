var SortUtil = {
  ...
  columnToIndexes: function (ind) {
    ...
    for (var i=1; i<this.cells.length; i++) {
      indexOfValue = this.columnValueToIndex[ind][this.getRow(i).getCell(ind).getValue()];
  ...
  indexesToColumn: function (ind) {
    ...
    for (var i=1; i<this.cells.length; i++) {
      this.getRow(i).getCell(ind).setValue(this.columnIndexToValue[ind][this.getRow(i).getCell(ind).getValue()]);
    }
  ...
  configureRowCol: function () {
    ...
    for (i=0; i<this.height; i++) {
      for (j=0; j<this.width; j++) {
        this.getRow(i).getCell(j).row = i;
        this.getRow(i).getCell(j).col = j;
      }
    }
