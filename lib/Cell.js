...
var Cell = function () {
   ...
   * @param {Grid} grid - A reference to the containing grid
   * @returns {string}
   */
  this.getFormattedValue = function (grid) {
    switch (this.type) {
      ...
      case DataType.Date:
        return '<input type="text" class="invisible Grid-datepicker" blotter-format="'+this.format+'"' +
            'readonly="readonly" value="' + moment(this.getRawValue()).format(this.format) + '" />';
      case DataType.Boolean:
        return '<input type="checkbox" class="Grid-checkbox" '+(this.getRawValue() ? 'checked="checked"' : '')+' />';
      default:
