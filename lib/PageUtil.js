...
var PageUtil = {
  ...
  addPages: function () {
    $(this).append('\
    <div class="Grid-pages noselect">\
      Page <b class="Grid-currentpage">' + this.currentPage + '</b> of ' + this.numberOfPages + '\
      <div class="Grid-page-prev ' + (this.currentPage==1?'Grid-page-disabled':'') + '"></div>\
      <div class="Grid-page-next ' + (this.currentPage==this.numberOfPages?'Grid-page-disabled':'') + '">\
      </div>\
    </div>');
    $(this).find('.Grid-currentpage').css('width', this.numberOfPages.toString().length * 8);
  },
  ...
  events: function () {
    ...
    $(this).find('.Grid-page-prev').on('click', PageUtil.prevPage.bind(this));
    $(this).find('.Grid-page-next').on('click', PageUtil.nextPage.bind(this));
    
    // Holding down the mouse on previous should increase speed of moving back pages
    $(this).find('.Grid-page-prev').on('mousedown', function () {
    ...
    $(this).find('.Grid-page-next').on('mousedown', function () {
